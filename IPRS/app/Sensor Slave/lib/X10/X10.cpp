...
X10::X10(volatile uint8_t clock_pin, volatile uint8_t X10_read, volatile uint8_t X10_write, uint8_t* address, size_t addressSize, char unit)
{
    ...
    DDRD &= ~clock_pin_;    //Set pin asignments
    DDRD &= ~X10_read_;
    DDRD |= X10_write_;
...
bool X10::readHalfBit() const
{
    ...
    while (true) // While reading
    {
        if (PIND & clock_pin_)
        {
            while (PIND & clock_pin_)   //While clock pin is high set bit true if true is send on X10
            {
                if (PIND & X10_read_)
                {
                    bit = true;
...
void X10::writeBit(uint8_t bit) const
{
    while (true)
    {
        if (PIND & clock_pin_)      //Wait for pulse on clock_pin
        {
            if (bit == 1)            //If sending a 1 write 10 half bits
            {
                PORTD |= X10_write_;
                while(PIND & clock_pin_)
                {
                }
                PORTD &= ~X10_write_;
                while((PIND & clock_pin_) == 0)
                {
                }
                while(PIND & clock_pin_)
                {
